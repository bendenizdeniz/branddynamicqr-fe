...
const OwnerManagement: React.FC = () => {
  ...
  return (
    <div
      className="p-6 space-y-6 bg-gray-50/50 min-h-screen"
      onClick={() => setActiveMenuId(null)}
    >
      {/* Header & Search Bölümleri Aynen Kalıyor... */}
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-bold text-gray-800 tracking-tight">
          Şirket (Owner) Yönetimi
        </h1>
        <button
          onClick={() => {
            setSelectedOwner({
              ...
              official_name: "",
              address: "",
              ext_id: `OWN_${Date.now()}`,
              is_deleted: false,
            } as Owner);
            setModalMode("create");
            setIsModalOpen(true);
          }}
          className="bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2.5 rounded-xl flex items-center gap-2 font-bold shadow-lg transition-all active:scale-95"
        >
          ...
          >
            <td className="px-6 py-4">
              <div className="flex items-center gap-3">
                <div
                  className={`w-10 h-10 rounded-2xl flex items-center justify-center font-bold text-lg ${owner.is_deleted ? "bg-gray-100 text-gray-400" : "bg-indigo-100 text-indigo-600"}`}
                >
                  {owner.name.charAt(0)}
                </div>
            ...
            <td className="px-6 py-4 text-left">
              <div className="flex gap-2 font-bold text-[10px]">
                <span className="bg-blue-50 text-blue-700 px-2 py-1 rounded-lg border border-blue-100">
                  {owner._count?.brands || 0} MARKA
                </span>
                <span className="bg-purple-50 text-purple-700 px-2 py-1 rounded-lg border border-purple-100">
                  {owner._count?.identities || 0} KİMLİK
                </span>
              </div>
            </td>
            <td className="px-6 py-4">
              <div
                className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full ${!owner.is_deleted ? "bg-green-50 text-green-600" : "bg-red-50 text-red-600"}`}
              >
                <div
                  className={`w-1.5 h-1.5 rounded-full ${!owner.is_deleted ? "bg-green-500" : "bg-red-500"}`}
                />
                <span className="text-xs font-bold">
                  {!owner.is_deleted ? "Aktif" : "Pasif"}
                </span>
              </div>
            </td>
      ...
      {activeMenuId &&
        createPortal(
          ...
          >
            ...
            <button
              onClick={() => {
                const owner = owners.find((o) => o.id === activeMenuId);
                if (owner) {
              ...
              className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-gray-600 hover:bg-gray-50 font-medium text-left"
            >
            ...
            <button
              ...
              className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-gray-600 hover:bg-gray-50 font-medium text-left border-t border-gray-50"
            >
            ...
            <button
              onClick={() => {
                if (confirm("Silinsin mi?"))
                  api.delete(`/owners/${activeMenuId}`).then(fetchOwners);
                setActiveMenuId(null);
              }}
              className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-red-600 hover:bg-red-50 font-bold text-left border-t border-gray-50"
            >
              <Trash2 size={16} /> Sil
            </button>
          </div>,
      ...
      <Modal
        ...
        title={modalMode === "create" ? "Yeni Şirket" : "Şirket Bilgileri"}
      >
        {selectedOwner && (
          <form
            onSubmit={(e) => {
          ...
          >
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-1 text-left">
                ...
                <input
                  ...
                  value={selectedOwner.vkn || ""}
                  onChange={(e) =>
                    setSelectedOwner({ ...selectedOwner, vkn: e.target.value })
                  }
                />
              </div>
              <div className="col-span-2 space-y-1 text-left">
                ...
                <input
                  disabled={modalMode === "view"}
                  className="w-full p-2.5 bg-gray-50 border border-gray-200 rounded-xl outline-none"
                  value={selectedOwner.official_name || ""}
                  placeholder="Ticari sicil ünvanı..."
                  onChange={(e) =>
                    setSelectedOwner({
                      ...selectedOwner,
            ...
            {modalMode !== "view" && (
              <div className="flex justify-end gap-3 pt-4 border-t sticky bottom-0 bg-white">
                ...
                <button
                  type="submit"
                  className="bg-indigo-600 text-white px-8 py-2.5 rounded-xl font-bold flex items-center gap-2 shadow-lg"
                >
                  <Save size={18} /> Kaydet
                </button>
              </div>
