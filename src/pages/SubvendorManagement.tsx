...
const SubvendorManagement: React.FC = () => {
  ...
  const [selectedSubvendor, setSelectedSubvendor] = useState<Subvendor | null>(
    null,
  );
  ...
  const fetchSubvendors = useCallback(async () => {
    setLoading(true);
    try {
      // GET yerine POST kullanıyoruz çünkü parametreleri body ile göndereceğiz
      ...
      >
        {subvendors.map((sub) => (
          ...
          >
            ...
            <td className="px-6 py-4">
              <div
                className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full w-fit ${sub.is_active ? "bg-green-50 text-green-600" : "bg-red-50 text-red-600"}`}
              >
                <div
                  className={`w-1.5 h-1.5 rounded-full ${sub.is_active ? "bg-green-500" : "bg-red-500"}`}
                />
            ...
            <td className="px-6 py-4 text-right relative">
              ...
              {activeMenuId === sub.id && (
                <div className="absolute right-12 top-4 w-44 bg-white rounded-xl shadow-xl border border-gray-100 z-30 py-2 animate-in fade-in slide-in-from-right-2">
                  <button
                    onClick={() => {
                      setSelectedSubvendor(sub);
                      setModalMode("view");
                      setIsModalOpen(true);
                    }}
                    className="w-full flex items-center gap-3 px-4 py-2 text-sm text-gray-600 hover:bg-gray-50 font-medium"
                  >
                  ...
                  <button
                    onClick={() => handleStatusToggle(sub.id, sub.is_active)}
                    className="w-full flex items-center gap-3 px-4 py-2 text-sm text-orange-600 hover:bg-orange-50 font-medium"
                  >
                    <Power size={16} />{" "}
                    {sub.is_active ? "Pasifleştir" : "Aktifleştir"}
                  </button>
          ...
          >
            <div className="grid grid-cols-2 gap-x-6 gap-y-4">
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setSelectedSubvendor({
                      ...selectedSubvendor,
                      employee_volume: Number(e.target.value),
                    })
                  }
                />
              </div>
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  className="w-full p-2.5 bg-gray-100 border border-gray-200 rounded-xl opacity-70"
                  value={
                    selectedSubvendor.is_franchise ? "Franchise" : "Öz Mal"
                  }
                />
              </div>
              ...
              <div className="space-y-1">
                ...
                <div
                  className={`p-2.5 rounded-xl border font-bold text-center ${selectedSubvendor.is_active ? "bg-green-50 border-green-200 text-green-600" : "bg-red-50 border-red-200 text-red-600"}`}
                >
            ...
            {modalMode === "edit" && (
              <div className="flex justify-end gap-3 pt-4 border-t sticky bottom-0 bg-white">
                ...
                <button
                  type="submit"
                  className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-xl flex items-center gap-2 font-semibold shadow-sm transition-all"
                >
